...
Part&nbsp;`r if(knitr::is_latex_output()){"\\ref{part-subpopulation-discovery-in-high-dimensional-data}"}else{"I"}` covers methods for discovering subpopulations in high-dimensional data. 
Part&nbsp;`r if(knitr::is_latex_output()){"\\ref{part-exploiting-dynamics}"}else{"II"}` focuses specifically on temporal aspects of medical datasets and provides approaches that extract informative representations from latent temporal data. 
Part&nbsp;`r if(knitr::is_latex_output()){"\\ref{part-post-mining-for-interpretation}"}else{"II"}` addresses post-hoc analysis of machine learning models and includes solutions to derive model-, observation-, and subpopulation-level insights from otherwise "opaque" black boxes. 
